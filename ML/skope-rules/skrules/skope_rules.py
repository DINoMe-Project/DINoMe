...
from .xgboost_rules import xgbtree_rule_perf,xgbtree_to_rules
...
class SkopeRules(BaseEstimator):
    ...
    def fit_xgbmodel(self, pddata,xgbmodel, sample_weight=None):
        feature_names_=xgbmodel.feature_names
        self.feature_dict_ = {BASE_FEATURE_NAME + str(i): feat
                                  for i, feat in enumerate(feature_names_)}
        paths=xgbmodel.trees_to_dataframe();
        rules_=[]
        self.rules_ = {}
        for i in range(xgbmodel.best_ntree_limit):
            rules_from_tree = xgbtree_to_rules(paths,i)
            scores=[]
            #for i,rule in enumerate(rules_from_tree):
            #    scores.append(xgbtree_rule_perf(rule,pddata,pddata['Y']))
            #rules_from_tree=[(rules_from_tree[i],scores[i]) for i in range(len(rules_from_tree))]
            rules_=rules_+rules_from_tree
        #rules_ = [
        ...
        for rule in rules_:
            if rule not in scores:
                start = timeit.timeit()
                scores[rule]=[]
                scores[rule]=xgbtree_rule_perf(rule,pddata,pddata['Y'],sample_weight)
                print("eval rule",rule, timeit.timeit()-start)
            score=scores[rule]
            if score[0] >= self.precision_min and score[1] >= self.recall_min:
                if rule in self.rules_:
                    self.rules_[rule]=(score[0],score[1],self.rules_[rule][2]+1)
                else:
                    self.rules_[rule] = score+ (1,)

        self.rules_ = sorted(self.rules_.items(),
                             key=lambda x: (x[1][0], x[1][1]), reverse=True)
        print("dedup rule")
        # Deduplicate the rule using semantic tree
        self.rules_before_dep=self.rules_.copy()
    ...
    def _find_similar_rulesets(self, rules):
        ...
        print("end split_with_best_feature")
